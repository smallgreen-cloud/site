...
<section class="section manifesto-purpose"><div class="shell"><div class="section-head"><div><p class="kicker">01 / {text("PURPOSE" if lang == "en" else "目的")}</p><h2>{title_lines(["What we are making possible" if lang == "en" else "我們要讓什麼成為可能"])}</h2></div><p class="section-intro">{text(purpose_intro)}</p></div><div class="manifesto-purpose-grid"><article><h3>{text(mission_label)}</h3><p>{text(mission)}</p></article><article><h3>{text(vision_label)}</h3><p>{text(vision)}</p></article></div></div></section>
